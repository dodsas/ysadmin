...
//
// 응답 경로 보호:
//   1) 동시에 여러 호출이 들어오면 같은 sweep promise 를 공유 — 여러 컴퓨터
//      상태 확인이 병렬로 들어와도 sweep 한 번만 실행.
//   2) 직전 sweep 가 SWEEP_TTL_MS 이내에 끝났으면 다시 돌지 않음. ARP 테이블은
//      분 단위로 유효하니 짧은 TTL 이면 정확도 손실 없이 응답을 단축.
const SWEEP_TTL_MS = Number(process.env.LAN_SWEEP_TTL_MS || 90_000);
let sweepInflight = null;
let lastSweepEndedAt = 0;

async function pingSweep() {
  if (sweepInflight) return sweepInflight;
  if (Date.now() - lastSweepEndedAt < SWEEP_TTL_MS) {
    logger.info('lan', 'sweep 캐시 히트 — 스킵', {
      msSinceLast: Date.now() - lastSweepEndedAt,
    });
    return true;
  }
  sweepInflight = (async () => {
    try {
      return await runPingSweep();
    } finally {
