import { readFile, writeFile, mkdir } from 'node:fs/promises';
import { dirname, resolve } from 'node:path';
import { randomUUID } from 'node:crypto';

const DATA_DIR = resolve(process.cwd(), 'data');
const DATA_FILE = resolve(DATA_DIR, 'targets.json');

// 핑 주기 — 대상별 설정. 기본값은 PING_INTERVAL_MS(없으면 10분).
// 상한은 하루(86,400초), 하한은 스케줄러 base tick(30초)에 맞춘다.
export const DEFAULT_INTERVAL_MS = Number(process.env.PING_INTERVAL_MS ?? 10 * 60 * 1000);
const MIN_INTERVAL_MS = 30 * 1000;
const MAX_INTERVAL_MS = 24 * 60 * 60 * 1000;

export function clampInterval(ms) {
  const n = Number(ms);
  if (!Number.isFinite(n) || n <= 0) return DEFAULT_INTERVAL_MS;
  return Math.max(MIN_INTERVAL_MS, Math.min(MAX_INTERVAL_MS, Math.round(n)));
}

let cache = null;
let writeQueue = Promise.resolve();

function applyDefaults(t) {
  if (!t.basicAuth) t.basicAuth = null;
  if (!t.sso) t.sso = null;
  if (t.intervalMs == null) t.intervalMs = DEFAULT_INTERVAL_MS;
  return t;
}

async function ensureLoaded() {
  if (cache) return cache;
  try {
    const raw = await readFile(DATA_FILE, 'utf8');
    cache = JSON.parse(raw);
    if (!Array.isArray(cache)) cache = [];
    cache.forEach(applyDefaults);
  } catch (err) {
    if (err.code === 'ENOENT') {
      cache = [];
    } else {
      throw err;
    }
  }
  return cache;
}

async function persist() {
  await mkdir(dirname(DATA_FILE), { recursive: true });
  await writeFile(DATA_FILE, JSON.stringify(cache, null, 2), 'utf8');
}

function enqueueWrite() {
  writeQueue = writeQueue.then(persist).catch((err) => {
    console.error('[store] persist failed:', err);
  });
  return writeQueue;
}

// hot path — pinger 의 주기 체크 + 프론트 폴링이 매번 호출. 모든 호출자가
// read-only 로 쓰므로 raw cache 를 그대로 반환 (얕은 복사 비용 제거).
export async function listTargets() {
  await ensureLoaded();
  return cache;
}

export async function addTarget({ url, label }) {
  await ensureLoaded();
  const normalized = normalizeUrl(url);
  const exists = cache.find((t) => t.url === normalized);
  if (exists) {
    const err = new Error('이미 등록된 URL입니다.');
    err.code = 'DUPLICATE';
    throw err;
  }
  const target = {
    id: randomUUID(),
    url: normalized,
    label: (label && String(label).trim()) || hostnameOf(normalized),
    basicAuth: null,
    sso: null,
    intervalMs: DEFAULT_INTERVAL_MS,
    createdAt: new Date().toISOString(),
    status: 'unknown',
    lastCheckedAt: null,
    lastLatencyMs: null,
    lastError: null,
    lastStatusCode: null,
  };
  cache.push(target);
  await enqueueWrite();
  return { ...target };
}

export async function removeTarget(id) {
  await ensureLoaded();
  const idx = cache.findIndex((t) => t.id === id);
  if (idx === -1) return false;
  cache.splice(idx, 1);
  await enqueueWrite();
  return true;
}

export async function updateTargetStatus(id, patch) {
  await ensureLoaded();
  const target = cache.find((t) => t.id === id);
  if (!target) return null;
  Object.assign(target, patch);
  await enqueueWrite();
  return { ...target };
}

export async function updateTarget(id, patch) {
  await ensureLoaded();
  const t = cache.find((x) => x.id === id);
  if (!t) return null;
  if (patch.label !== undefined) {
    const next = String(patch.label).trim();
    if (next) t.label = next;
  }
  if (patch.basicAuth !== undefined) {
    if (patch.basicAuth === null) {
      t.basicAuth = null;
    } else {
      const username = String(patch.basicAuth.username || '').trim();
      const password = typeof patch.basicAuth.password === 'string' ? patch.basicAuth.password : '';
      t.basicAuth = username ? { username, password } : null;
    }
  }
  if (patch.intervalMs !== undefined) {
    t.intervalMs = clampInterval(patch.intervalMs);
  }
  if (patch.sso !== undefined) {
    if (!patch.sso || patch.sso.enabled === false) {
      t.sso = null;
    } else {
      const secret = String(patch.sso.secret || '').trim();
      if (!secret) {
        const err = new Error('SSO 시크릿이 필요합니다.');
        err.status = 400;
        throw err;
      }
      const endpoint = String(patch.sso.endpoint || '/sso').trim() || '/sso';
      const subject = String(patch.sso.subject || '').trim();
      const ttlSec = Number.isFinite(Number(patch.sso.ttlSec)) ? Number(patch.sso.ttlSec) : 30;
      t.sso = {
        enabled: true,
        secret,
        endpoint: endpoint.startsWith('/') ? endpoint : `/${endpoint}`,
        subject: subject || null,
        ttlSec: Math.max(5, Math.min(600, ttlSec)),
      };
    }
  }
  await enqueueWrite();
  return { ...t };
}

export async function getTarget(id) {
  await ensureLoaded();
  const target = cache.find((t) => t.id === id);
  return target ? { ...target } : null;
}

export async function reorderTargets(orderedIds) {
  await ensureLoaded();
  if (!Array.isArray(orderedIds)) {
    throw new Error('순서는 ID 배열이어야 합니다.');
  }
  const byId = new Map(cache.map((t) => [t.id, t]));
  const seen = new Set();
  const next = [];
  for (const id of orderedIds) {
    const t = byId.get(id);
    if (!t || seen.has(id)) continue;
    next.push(t);
    seen.add(id);
  }
  for (const t of cache) {
    if (!seen.has(t.id)) next.push(t);
  }
  cache = next;
  await enqueueWrite();
  return cache.map((t) => ({ ...t }));
}

function normalizeUrl(input) {
  if (typeof input !== 'string') throw new Error('URL은 문자열이어야 합니다.');
  const trimmed = input.trim();
  if (!trimmed) throw new Error('URL이 비어 있습니다.');
  let candidate = trimmed;
  if (!/^https?:\/\//i.test(candidate)) candidate = `https://${candidate}`;
  const u = new URL(candidate);
  return u.toString().replace(/\/$/, '');
}

function hostnameOf(url) {
  try {
    return new URL(url).hostname;
  } catch {
    return url;
  }
}
